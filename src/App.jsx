...
// Иконки по умолчанию для левого меню (из AdmiralDS/icons)
// icon-0 до icon-6: верхние 7 иконок
// icon-7: кастомная (только локальная загрузка, без дефолта)
// icon-8, icon-9: нижние иконки (Help, Exit)
const DEFAULT_MENU_ICONS = {
  ...
  // icon-7: НЕТ дефолта - кастомная иконка для локальной загрузки
  // Нижние 2 иконки с дефолтами (Help, Exit)
...
// Дефолтные иконки для внутреннего меню (Избранные продукты, Акции)
const DEFAULT_NAV_MENU_ICONS = {
  // Star Outline для "Избранные продукты" - цвет #717681
  ...
  // Bonus Ruble Outline для "Акции" - цвет #717681
...
// Дефолтные иконки для клиентского сайдбара
const DEFAULT_CLIENT_ICONS = {
  // Safety Certificate Solid для телефона - цвет #32C665
  ...
  // Government Solid для паспортных данных - цвет #9F1853
...
export default function App() {
  ...
  // Транслитерация кириллицы в латиницу для создания безопасных ID
  ...
  // Функция для создания безопасного ID из строки (с поддержкой кириллицы)
  ...
  // Уникальный ID карточки: "card-" + индекс (гарантированно уникальный)
  ...
  const [uploadedMenuIcons, setUploadedMenuIcons] = useState({}); // { menuItemId: svgContent } для "Избранные продукты" и "Акции"
  ...
  const uploadContextRef = useRef(null); // контекст на момент открытия модалки — чтобы при выборе файла (async) использовать тот же slot
  const [visibleTabsCount, setVisibleTabsCount] = useState(tabs.length);
  const [showMoreMenu, setShowMoreMenu] = useState(false);
  const [showCounters, setShowCounters] = useState(false); // Показывать счётчики по клику на колокольчик
  const tabsContainerRef = useRef(null);
  const tabsRefs = useRef([]);
  
  // Загрузка всех изображений из localStorage при инициализации
  useEffect(() => {
    ...
    // Все картинки хранятся напрямую в localStorage как SVG контент
    ...
    // Объединяем дефолтные иконки с пользовательскими (пользовательские имеют приоритет)
  ...
  const cards = [
    ...
    { title: 'Периодический перевод', bonus: 'Не начисляются', bonusGray: true },
  ...
  const saveToStorage = (key, id, content, setState) => {
    try {
      ...
      alert('Ошибка сохранения: возможно, превышен лимит localStorage');
      return false;
    }
  };

  // Обработчик загрузки SVG файла — всё сохраняется напрямую в localStorage
  ...
  // Открытие модального окна. Сохраняем контекст в ref, чтобы при выборе файла (async) использовался именно этот слот.
  ...
  useEffect(() => {
    const calculateVisibleTabs = () => {
      ...
      const moreButtonWidth = 130; // Ширина кнопки "Показать еще"
      let usedWidth = 0;
      let visibleCount = 0;
      ...
      // Если все табы помещаются, показываем все
      if (visibleCount >= tabs.length) {
        setVisibleTabsCount(tabs.length);
      } else {
        // Оставляем место для кнопки "Показать еще"
    ...
    <>
      ...
      <div style={{
        ...
        <nav style={{
          ...
          {/* Bottom icons - 3 штуки (icon-7, icon-8, icon-9) */}
          {/* icon-7: Кастомная иконка (только локальная загрузка) */}
          <div 
            className="nav-icon"
            style={{
              width: '48px', 
        ...
        <aside className="ios-scroll" style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {[
                  { text: 'Распечатать титульный лист', badge: 'Новая', badgeColor: '#32C665', textColor: 'white' },
                  { text: 'Обновить сведения по СЮЛ', badge: 'В работе', badgeColor: '#33B1FF', textColor: 'white' },
                ].map((task, i) => (
                  <div key={i} style={{ 
                    display: 'flex', 
                    alignItems: 'center',
            ...
            <div style={{
              ...
              >
                {tabs.map((tab, idx) => {
                  ...
                  const showTabCounter = showCounters && idx < 4; // Счётчики на первых 4 табах
                ...
                {/* Кнопка "Показать еще" если есть скрытые табы */}
            ...
            <div className="ios-scroll" style={{
              ...
              <div className="cards-grid">
                {(activeTab === 4 ? cards.slice(0, 4) : cards).map((card, idx) => (
                  ...
                  >
                    {/* Контейнер иллюстрации и метки - занимает всю ширину */}
                    <div style={{
                      width: '100%',
                      display: 'flex',
                      alignItems: 'flex-start',
                      ...
                      {/* Бейдж "Новое" справа от иллюстрации */}
                      {card.badge && (
                        <span style={{
                          background: '#EF4444',
                          color: 'white',
        ...
        <footer style={{
          ...
          right: '24px', // выравнивание по правому краю клиентского сайдбара
          height: '56px',
          background: 'transparent',
          display: 'flex',
          alignItems: 'center',
          ...
          <div>
            ...
            <div style={{ fontSize: '14px', fontWeight: '500', color: '#2563EB', cursor: 'pointer' }}>Сфера. Обращения</div>
          </div>
          <div style={{ marginLeft: 'auto' }}>
            <div style={{ fontSize: '12px', color: '#9CA3AF', marginBottom: '2px' }}>Руководство пользователя в ФРКК</div>
            <div style={{ fontSize: '12px', color: '#9CA3AF' }}>Последний вход 25.08.2026</div>
          </div>
        </footer>

        {/* Модальное окно для загрузки SVG */}
        {uploadModal.open && (
          <div 
            style={{
              position: 'fixed',
          ...
          >
            ...
            >
              <h2 style={{
                ...
                Загрузить SVG {
                  uploadModal.type === 'icon' ? 'иконку' : 
                  uploadModal.type === 'menu-icon' ? 'иконку меню' :
                  uploadModal.type === 'client-icon' ? 'иконку клиента' :
                  'изображение'
                }
              </h2>
              <p style={{
                ...
                {uploadModal.type === 'icon' 
                  ? `Выберите SVG файл для иконки`
                  : uploadModal.type === 'menu-icon'
                  ? `Выберите SVG файл для "${navMenuItems.find(item => !item.divider && getMenuItemId(item) === uploadModal.menuItemId)?.label || ''}"`
                  : uploadModal.type === 'client-icon'
                  ? `Выберите SVG файл для ${uploadModal.clientIconType === 'phone-check' ? 'иконки телефона' : 'иконки номера телефона'}`
                  : `Выберите SVG файл для карточки "${cards.find((card, i) => getCardId(card, i) === uploadModal.cardId)?.title || ''}"`
                }
              </p>
              <label style={{
                display: 'block',
